...
class YoloWorldDetector:
    ...
    def predict_boxes(self, image: np.ndarray, prompt: str, device: str = "cuda", conf: float | None = None) -> list[list[float]]:
        ...
        use_conf = conf if conf is not None else 0.4
        results = self.model.predict(bgr_image, device=device, verbose=False, conf=use_conf)
